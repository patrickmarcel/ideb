...
public class userThread implements Runnable {
	...
	public void run(){
		...
		for(Session s : us.keySet()){
			...
			String fileName=TestInterestingnessMT.pathToResult +"RESULT-" +timestamp + "-" +s.getMetadata("filename")+".csv";
			...
			for(Query q : s.getQueryList()){
				...
				for(EAB_Cell c : q.getResult().getCellList().getCellCollection()){
					...
					//Thread t=new Thread(new  cellThread(c,u,q, sessionName, username, queryPos, queryLabel, sessionLabel));
					//t.start();
				...
				System.out.println("Memory consumption: " +  (Runtime.getRuntime().totalMemory()/1024)/1024 + " MB");
			...
			System.out.println("in session: " + currentSession.getMetadata("filename"));
